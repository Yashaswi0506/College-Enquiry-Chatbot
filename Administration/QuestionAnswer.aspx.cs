...
public partial class Administration_QuestionAnswer : System.Web.UI.Page
{
     ...
     public class Porter2
    {
     ...
     protected void BtnSubmit_Click(object sender, EventArgs e)
     {
         ...
         string exceptions = "who where why when how what which whose whom with within without regard yet to mgms mgm's colleges college's so though while if even wherever rather wheather only well than through till towards under underneath until upon via behind below beneath beside off onto over considering consider per prior round between beyond but  much many mgm in case college details long far are the  it old come about against along is the of an by because means behalf other close despite down during due except follow forward futher inside include into inspite instead like near next as at please by from tell me i am is or our let know do did does give some help regarding was since across after before along anti around per ";
             ...
             cmd.CommandText = "insert into Tbl_Info (Question,Answer)values(@question,@answer)";
             cmd.Parameters.Add("@question", SqlDbType.NVarChar).Value = TextBox1.Text;
             cmd.Parameters.Add("@answer", SqlDbType.NVarChar).Value = TxtAnswer.Text;
             try
             {
                 con.Open();
                 cmd.ExecuteNonQuery();
                 LblStatus.Text = "Inforation Updated";
                 con.Close();
                 TxtQuestion.Text = "";
                 TxtAnswer.Text = "";
             }
